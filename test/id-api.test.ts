...
describe('IDapi', () => {
  ...
  describe('#submit_job', () => {
    it('should ensure that the partner_params are present', async () => {
      ...
      // @ts-ignore
      await expect(instance.submit_job(null, {})).rejects.toThrow(
    ...
    ['user_id', 'job_id', 'job_type'].forEach((key) => {
      ...
      it(`should ensure that partner_params contains ${key}`, async () => {
        ...
        await expect(instance.submit_job(partner_params, {})).rejects.toThrow(
          new Error(
    ...
    it('should ensure that the id_info object is not empty or nil', async () => {
      ...
      await expect(
        ...
        new Error('Please make sure that id_info not empty or nil'),
      );
    });
    ...
    it('should ensure that the id_info object contains a valid id_number as a string', async () => {
      ...
      await expect(
        instance.submit_job(
          { user_id: '1', job_id: '1', job_type: 5 },
          { id_number: '' },
        ),
      ).rejects.toThrow(
        new Error('Please provide an id_number in the id_info payload'),
      );
    });
    ...
    it('should ensure that the the job id is set to 5', async () => {
      ...
      // @ts-ignore
      await expect(instance.submit_job(partner_params, null)).rejects.toThrow(
        new Error(
          'Please ensure that you are setting your job_type to 5 or 7 to query ID Api',
        ),
      );
    });
